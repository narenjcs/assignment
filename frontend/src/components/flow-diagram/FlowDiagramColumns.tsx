...
/** Grid pattern and arrowhead marker shared by every edge in the diagram. */
function FlowDefs(): ReactElement {
  return (
    <defs>
...
/** One column heading with its cloud-coloured underline. */
  ...
  return (
    <>
      <text
        x={x}
        y={16}
        className={`${textClass} text-[10px] font-bold`}
        style={{ letterSpacing: '0.08em' }}
      >
        {label}
      </text>
      <line x1={x} y1={22} x2={endX} y2={22} stroke={color} strokeWidth={2} opacity={0.7} />
    </>
  );
}
